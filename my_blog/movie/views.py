...
class MovieCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        actual_objects = Movie.objects.filter(
            name=data["name"], produced_by=data["produced_by"]
        ).count()
        if actual_objects:
            messages.error(
                self.request,
                f"La pelicula {data['name']} - {data['produced_by']} ya está creada",
            )
            ...
            messages.success(
                self.request,
                f"Pelicula {data['name']} - {data['produced_by']} creada exitosamente!",
            )
